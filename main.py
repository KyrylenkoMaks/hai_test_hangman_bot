...
with open("telegramkey.txt", 'r') as file:
    hang_bot = telebot.TeleBot(file.read())
current_game = None
wordlist = ['удача', 'кілт', 'буран', 'будинок', 'розмарин', 'окуляри', 'фольга', 'батарея', 'піцца', 'вежа', 'цукерка',
            'гаманець', 'дзига', 'танго']
    ...
    def handle_guess(self, message):
        ...
        if letter in self.guessed_letters:
            ...
            if self.masked_word == self.word:
                current_game = None
                hang_bot.send_message(message.chat.id,
                                      f'Молодець! Гру завершено!\nПравильне слово: {self.word}\nЯкщо хочеш зіграти ще, відправляй /play')
            elif letter in self.word:
                hang_bot.send_message(message.chat.id,
                                      f'Гарна здогадка! Тепер слово виглядає так:\n{self.masked_word}\nПродовжуй в тому ж дусі!')
            else:
                self.attempts -= 1
                if self.attempts == 0:
                    current_game = None
                    hang_bot.send_message(message.chat.id,
                                          f'Жаль, але ти програв. Правильне слово: {self.word}.\nЯкщо хочеш зіграти ще - відправляй /play')
                else:
                    hang_bot.send_message(message.chat.id,
                                          f'Такої літери немає в слові. Залишилось помилок - {self.attempts}')
